...
- **主页口径 = 虚拟网络 + 地址（不是"房间"）**：Tailscale 是三层覆盖网，**不转发局域网广播/组播**（tailscale#11134），所以游戏**不会**自动扫描出对方大厅，必须在游戏里用「直接连接 / 通过 IP 加入」填 `100.x.x.x:端口`。主页因此分五段：①联机环境卡（状态 + 我的地址 + 同一虚拟网络里的设备 `_status.Peers` + 「游戏里不会自动出现房间」提示 `BroadcastHint`）②健康告警 ③和朋友连起来（我当主机 / 我加入朋友）④我要玩的游戏（游戏卡点一下直接进主机向导并预选）⑤最近联机 + 原理说明。「邀请/邀请码」是必需的（解决朋友入网授权），但 UI 一律不用「开房/房间」这套词（游戏内的房间/世界概念除外）。
