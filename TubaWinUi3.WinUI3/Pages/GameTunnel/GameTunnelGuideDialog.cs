...
public sealed class GameTunnelGuideDialog
{
    ...
    public GameTunnelGuideDialog(XamlRoot xamlRoot)
    {
        ...
        body.Children.Add(Expander("为什么在游戏里看不到朋友的房间？",
            "因为 Tailscale 是「三层」虚拟网络：只转发点对点的数据，不转发游戏发出的局域网广播（组播）。所以哪怕你们已经在同一个虚拟网络里，游戏也扫描不到对方的大厅——这不是没连上。\n"
            + "正确做法是在游戏里选「直接连接 / 通过 IP 加入」（不同游戏叫法不同：泰拉瑞亚是「通过 IP 加入」、我的世界是「直接连接」、帕鲁是「加入多人游戏（专用服务器）」），填对方的 100.x 地址和端口。\n"
            + "少数游戏的联机走平台自己的通道（例如 Steam 好友邀请），那种不受这条限制。"));

        body.Children.Add(Expander("朋友加入了，但游戏里连不上",
            "按顺序检查：\n① 主机的游戏还开着吗？本工具主页会显示「游戏正在监听端口」。\n② 朋友是真的加入成功了吗？让他看工具箱的「网络检测」里的状态。\n③ 游戏里是用「直接连接 / 通过 IP 加入」填的地址吗？（Tailscale 不转发局域网广播，游戏不会自动出现对方的房间。）\n④ 双方做一次「测试与对方的连接」，看看是连不通还是连得通但游戏不认（后者通常是版本不一致）。\n⑤ 个别安全软件会拦截虚拟网卡，让朋友临时关掉网络防护再试一次。"));
